...
02. Add Group for Write Access

    ```console
    groupadd -g 2000 prezto
    ```

03. Clone `ecool` branch to prezto location and set permissions
...
04. Add runcoms to `/etc/skel` + `/root` for default `users`/`root` and link `/opt/.zprezto` to home dirs
...
05. Set Zsh as `root` default shell:
